...
public class SerialRXTX {
    ...
    public static class SerialReader implements SerialPortEventListener {
        ...
        public void serialEvent(SerialPortEvent arg0) {
            ...
            try {
                ...
                //System.out.println(new String(buffer));   //注释修改
                String string=new String(buffer);
                //--将数据发往服务器--
                try {
        			DatagramSocket ds = new DatagramSocket();
        			byte[] buf = new byte[1024];
        			DatagramPacket dp = new DatagramPacket(string.getBytes(), string.length(), InetAddress.getByName("127.0.0.1"), 3000);
        			ds.send(dp);
        			ds.close();
            ...
            while (true) {
                try {
            		...
            		try {
            			DatagramSocket ds = new DatagramSocket();
            			byte[] buf = new byte[1024];
            			DatagramPacket dp = new DatagramPacket(commandString.getBytes(), commandString.length(), InetAddress.getByName("127.0.0.1"), 3000);
            			ds.send(dp);
            			ds.close();
            		} catch (SocketException e) {
            			e.printStackTrace();
    ...
    public static void main(String[] args) {
        listAvailablePorts();
        try {
            sendMessage(connect("COM3"), "P");   ///dev/ttyUSB0
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
